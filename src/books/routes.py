...
async def get_all_books(session: AsyncSession = Depends(get_session), token_details: dict = Depends(security)) -> List[dict]:
    books = await book_service.get_all_books(session)
    return books


@book_router.get("/user/{user_uid}", response_model=List[Book], dependencies=[Depends(role_checker)])
async def get_user_books(user_uid: str, session: AsyncSession = Depends(get_session), token_details: dict = Depends(security)) -> List[dict]:
...
async def create_a_book(book_data: CreateBook, session: AsyncSession = Depends(get_session), token_details: dict = Depends(security)) -> dict:
...
async def get_book(book_uid: str, session: AsyncSession = Depends(get_session), token_details: dict = Depends(security)) -> dict:
...
async def update_book(book_uid: str, book_update_data: UpdateBook, session: AsyncSession = Depends(get_session), token_details: dict = Depends(security)) -> dict:
...
async def delete_book(book_uid: str, session: AsyncSession = Depends(get_session), token_details: dict = Depends(security)) -> None:
